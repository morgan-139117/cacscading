...
//profile symbols start here
MAKE_SYMBOL(sched::set_adj_set);
MAKE_SYMBOL(sched::unset_adj_set);
MAKE_SYMBOL(sched::set_adj_running);
MAKE_SYMBOL(sched::unset_adj_running);
MAKE_SYMBOL(sched::set_adj_yield);
MAKE_SYMBOL(sched::unset_adj_yield);
MAKE_SYMBOL(sched::set_adj_n_yield);
MAKE_SYMBOL(sched::unset_adj_n_yield);
//profile symbols end here
...
//profiler initial values start here

bool __thread adj_set = false;
bool __thread adj_running = false;
bool __thread adj_yield = false;
bool __thread adj_n_yield = false;

//profiler initial values end here
                                    ...
                                    thread_runtime::duration preempt_after)
{
    ...
    if (!called_from_yield) {
        ...
        //try to figure out the initial quanta
		//now
	...
	std::string straft = std::to_string( std::chrono::duration_cast<std::chrono::microseconds>( tpaft.time_since_epoch()).count());
	sched::cpu::current()->profile_4.insert(TStrStrPair("now+preempt_after =>" + strnow,straft));
        }
    }
