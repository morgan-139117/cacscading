...
public:
	...
	void insert(TStrStrPair tp);
	void show();
};



inline void profilem::insert(TStrStrPair tp) {
	pro_map.insert(tp);
	counter++;
}

inline void profilem::show() {
	std::cout<<"show:"<<std::endl;
	for (TStrStrMap::iterator it = pro_map.begin(); it != pro_map.end(); ++it)
		std::cout <<counter<<" => "<< it->first << " => " << it->second << '\n';
}
inline long long int get_wall_time() {
    std::chrono::time_point<osv::clock::uptime> tpn = osv::clock::uptime::now();
      return  tpn.time_since_epoch().count();
}
//profiling class ends here

struct cpu : private timer_base::client {

	//profile variables start here
	 profilem profile_running_now;
	 profilem profile_running_del;
	 profilem profile_2;
	 profilem profile_3;
	 profilem profile_4;
...
//profile dependent variables & functions start here
extern bool __thread adj_set;
//extern bool __thread adj_timer;
extern bool __thread adj_running;
extern bool __thread adj_yield;
extern bool __thread adj_n_yield;

inline void set_adj_set(){
	adj_set = true;
}

inline void unset_adj_set(){
	adj_set = false;
}

inline void set_adj_running(){
	adj_running = true;
}

inline void unset_adj_running(){
	adj_running = false;
}

inline void set_adj_yield(){
	adj_yield = true;
}

inline void unset_adj_yield(){
	adj_yield = false;
}

inline void set_adj_n_yield(){
	adj_n_yield = true;
}

inline void unset_adj_n_yield(){
	adj_n_yield = false;
}

//profile dependent variables & functions end here
